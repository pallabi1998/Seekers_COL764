...
with open(f"./outputs/fullbm25.txt", 'w') as outfile:
    
    for qid in range(len(queries)):
        doc_score = np.array([])
        for ds in doc_scores[qid]:
            doc_score = np.concatenate((doc_score ,ds))
        print(doc_score)
        ...
        print(a)
        for i in range(500):
            outfile.write(f"{qid} Q0 {pids[a[i]]} {i} {doc_score[a[i]]} 1\n")
