            ...
            if in_p_tag:
                ...
                if "</p>" in line:
                    ...
                    if len(passages_no) % 20000000 == 0 and len(passages_no) > 0:
                        ...
                        tokenized_corpus = [tokenizer.tokenize(sentence) for sentence in passages_text]
                        
                        #tokenized_corpus = [doc.split(" ") for doc in passages_text]
                        bm25 = BM25Okapi(tokenized_corpus)
                        with open(f"bm25_bert_model_{LCNT}.pkl", "wb") as f:
                            pickle.dump(bm25, f)
                        del bm25
...
passages_no, passages_text = find_titles_in_file(wiki_path, max_titles=math.inf)
tokenized_corpus = [doc.split(" ") for doc in passages_text]
bm25 = BM25Okapi(tokenized_corpus)
